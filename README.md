...
当前发行版：**v1.3.0**。项目管理、Skills 分组、本地资产库、快捷入口设置及对应的 macOS/Windows 后台运行时会作为同一安装包更新，不需要分别安装。
...
- “资产控制台”作为内置快捷入口：在 Codex 主区域内打开纯本地多媒体资产库。每个项目可关联一个或多个文件夹并递归扫描，自动归类文本、图片、音频和视频；支持子分类、标签、项目内搜索、排序及 1–8 列自适应布局。
- 文本资产以可滚动卡片展示，双击可放大并编辑保存 Markdown、TXT、RTF 和 DOCX；旧式 `.doc` 可索引和预览元数据，但不直接改写。
