...
### 本次更新 · 模型竞技场 · 2026-09-12

- 在“资产控制台”后新增 **模型竞技场**，以可收起的右侧面板展示，保留当前对话。
- Wan 3.0、Seedance 2.0 / Fast / Mini / 2.5、MiniMax H3 六模型对比；三组 API 独立配置，公开包仅内置火山方舟、BytePlus LAS、阿里云百炼、MiniMax 四种官方协议，不含内部模型通道。H3 分辨率为 **768P、2K**。
- 从本地或资产库添加图片、音频、视频，用 `@图片1` 等引用；按所选模型共同上限校验，分辨率按模型独立设置，不静默删除素材。
- 先预览请求，再确认上传与生成；任务本地持久保存，提交结果不明时不自动重复发送。结果横向滚动比较，选中后可生成带模型标签的静音同屏对比或静音顺序拼接视频。

重新运行本仓库安装命令即可更新；真实供应商生成需配置自己的账号并确认。素材探测与合成需要本机 FFmpeg / FFprobe。公开包不附带账号、内部通道或生成额度。详见[配置、操作与验证范围](docs/MODEL-ARENA.md)。

### 上次更新 · Skills 管理升级 · 2026-09-11
...
- **Windows 保存兼容修复**：资产库配置遇到短暂文件占用时有界重试原子替换，不删除旧配置；持续占用仍明确报错。
