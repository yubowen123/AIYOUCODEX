...
const ALLOWED_OPEN_MODES = new Set(["internal", "browser", "in-app"]);
const CONFIG_KEYS = new Set(["schemaVersion", "shortcuts"]);
const SHORTCUT_KEYS = new Set(["id", "name", "url", "icon", "openMode", "keepAlive"]);
...
function normalizeShortcut(raw, index) {
  ...
  if (!ALLOWED_OPEN_MODES.has(raw.openMode)) {
    throw new TypeError(`${label}.openMode must be internal or browser or in-app`);
  }
  if (raw.keepAlive !== undefined && typeof raw.keepAlive !== "boolean") {
    throw new TypeError(`${label}.keepAlive must be a boolean`);
  }
  if (raw.keepAlive === true && raw.openMode === "browser") {
    throw new TypeError(`${label}.keepAlive is supported only for internal shortcuts or in-app shortcuts`);
  }
  ...
  return Object.freeze({
    ...
    ...(raw.keepAlive === true ? { keepAlive: true } : {}),
  });
}
...
export function managedShortcutsPath({
  ...
  if (platform === "win32") {
    ...
    return pathApi.join(localAppData, "CodexSidebarEnhancer", "Data", "managed-shortcuts.json");
  }
  if (platform === "darwin") {
    return pathApi.join(
      homeDir,
  ...
  const configHome = String(env?.XDG_CONFIG_HOME || "").trim() || pathApi.join(homeDir, ".config");
  return pathApi.join(configHome, "codex-sidebar-enhancer", "managed-shortcuts.json");
}
