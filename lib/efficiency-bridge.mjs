...
export class EfficiencyBridge {
  constructor(controller, { binding = EFFICIENCY_BINDING, resolver = "resolveEfficiencyRequest", publicErrorCodes = [], fallbackError } = {}) {
    this.controller = controller; this.binding = binding; this.resolver = resolver; this.publicErrorCodes = publicErrorCodes; this.fallbackError = fallbackError;
    this.off = []; this.contexts = new Set(); this.seen = new Set(); this.generation = 0;
  }
  async install(client) {
    ...
    const registerContext = (context) => {
      if (context?.auxData?.isDefault && context.auxData.frameId === this.frameId) {
        ...
        return client.send("Runtime.addBinding", { name: this.binding, executionContextId: context.id }).catch(() => {});
      }
    };
  ...
  async receive({ name, payload, executionContextId }, generation = this.generation) {
    if (generation !== this.generation || name !== this.binding || !this.contexts.has(executionContextId)
    ...
    catch (error) { response = { requestId: value.requestId, ok: false, error: this.publicErrorCodes.includes(error.code) ? error.message : this.fallbackError || (error.code === "EFFICIENCY_CONFLICT"
    ...
    await this.client.send("Runtime.evaluate", { contextId: executionContextId,
      expression: `window.__codexConversationPreviewInjection__?.[${JSON.stringify(this.resolver)}]?.(${JSON.stringify(response)})`, returnByValue: true }).catch(() => {});
  }
  dispose() {
    this.generation += 1;
