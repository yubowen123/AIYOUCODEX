...
const actions = new Set(["refresh", "saveScope", "resetScope", "saveContext", "resetContext", "summarizeContext",
  "previewContextExecution", "prepareContextExecution", "executeContext"]);
...
export function createEfficiencyController({ repository, readActiveContext, rootDir, store, executeContext, now = Date.now } = {}) {
  ...
  async function currentTarget() {
    ...
    const targetKey = createHash("sha256").update(JSON.stringify([active?.threadId || "", scope.threadId, scope.project?.key || ""])).digest("hex");
...
export class EfficiencyBridge {
  ...
  async receive({ name, payload, executionContextId }, generation = this.generation) {
    ...
    try { response = { requestId: value.requestId, ok: true, data: await this.controller.request(value) }; }
      ...
      ? "设置已被其他窗口修改，请重新加载后再保存。" : ["EFFICIENCY_TARGET_CHANGED", "EFFICIENCY_NEXT_STEP_REQUIRED", "EFFICIENCY_PREVIEW_CHANGED", "EFFICIENCY_EXECUTION_EXPIRED", "EFFICIENCY_EXECUTION_UNAVAILABLE"].includes(error.code)
