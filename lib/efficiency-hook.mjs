...
export function renderEfficiencyContext(view, { resolvedSkills, maxCharacters = EFFICIENCY_HOOK_CHAR_LIMIT } = {}) {
  ...
  const budget = Math.min(view.effective.contextBudget, maxCharacters - policy.length - 100);
...
export async function runEfficiencyHook(event, { store, rootDir, now = Date.now(), resolveThread, resolveSkills, contextReserve = 0 } = {}) {
  ...
  return withEfficiencyFileLock(selectedStore.hookPath, async () => {
    ...
    if (emit) {
      ...
      rendered = renderEfficiencyContext(view, { resolvedSkills, maxCharacters: Math.min(EFFICIENCY_HOOK_CHAR_LIMIT, EFFICIENCY_HOOK_CONTEXT_LIMIT - contextReserve) });
    }
