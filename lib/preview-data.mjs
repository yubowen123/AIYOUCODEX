...
import { parseRateLimitLines, parseTokenUsageLines } from "./usage-data.mjs";

const THREAD_ID_PATTERN = /([0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12})/i;
const EXACT_LOCAL_THREAD_ID_PATTERN = /^(?:local:)?([0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12})$/i;
...
async function readSessionWorkspace(filePath, expectedThreadId = "") {
  if (!filePath) return "";
  let handle;
  try {
    ...
    for (const line of header.split("\n")) {
      ...
      if (expectedThreadId && String(entry.payload?.id || "").toLowerCase() !== expectedThreadId) continue;
      const workspace = entry.payload?.cwd;
      if (typeof workspace !== "string" || !workspace.trim() || /[\u0000-\u001f]/u.test(workspace)) continue;
      if (!path.isAbsolute(workspace) && !path.win32.isAbsolute(workspace)) continue;
      return workspace;
    }
    return "";
  } catch {
...
function historyAttachmentNames(value) {
  const section = String(value || "").match(/#\s*Files mentioned by the user:\s*([\s\S]*?)(?:\n\s*Distinguish instructions|\n\s*##?\s*My request:|$)/i)?.[1] || "";
...
export function isHiddenConversationContent(value) {
  const text = String(value || "").trim().toLocaleLowerCase();
  return !text || HIDDEN_HISTORY_PREFIXES.some((prefix) => text.startsWith(prefix));
}

export function visibleConversationText(value, { user = false } = {}) {
  let text = String(value || "")
    .replace(/<oai-mem-citation>[\s\S]*?<\/oai-mem-citation>/gi, "")
    .trim();
  if (!text || isHiddenConversationContent(text)) return "";
  if (!user) return text;

  const attachments = historyAttachmentNames(text);
  const marker = text.match(/(?:^|\n)##?\s*My request:\s*\n?/i);
  if (marker?.index != null) text = text.slice(marker.index + marker[0].length).trim();
  const attachmentLabel = attachments.length ? `附件：${attachments.join("、")}` : "";
  return [attachmentLabel, text].filter(Boolean).join("\n\n");
}

export function parseConversationMessageEntry(entry) {
  ...
  const parts = (Array.isArray(entry.payload.content) ? entry.payload.content : [])
    .filter((item) => item && (item.type === "input_text" || item.type === "output_text"))
    .map((item) => visibleConversationText(item.text, { user: role === "user" }))
    .filter(Boolean);
  ...
  return {
    ...
    role,
    phase: role === "assistant" ? String(entry.payload.phase || "") : "",
    timestamp: timestampValue(entry.timestamp) ? new Date(timestampValue(entry.timestamp)).toISOString() : null,
    text,
  };
}

async function readConversationAppend(filePath, { start = 0, remainder = null, onEntry } = {}) {
  let lineParts = [];
  let lineBytes = 0;
  let lineRelevant = remainder?.relevant ?? null;
  if (Buffer.isBuffer(remainder?.buffer) && remainder.buffer.length) {
    lineParts = [remainder.buffer];
    lineBytes = remainder.buffer.length;
  }

  const finishPart = (part, complete) => {
    if (part.length && lineRelevant !== false) lineParts.push(part);
    lineBytes += part.length;
    if (lineRelevant == null && (complete || lineBytes >= 512)) {
      const prefix = Buffer.concat(lineParts, Math.min(lineBytes, 4096)).subarray(0, 4096).toString("utf8");
      if (prefix.includes('"type":"response_item"') && prefix.includes('"payload":{"type":"message"')) {
        lineRelevant = true;
      } else if (complete || lineBytes >= 4096) {
        lineRelevant = false;
        lineParts = [];
      }
    }
    if (!complete) return;
    if (lineRelevant === true) {
      let line = Buffer.concat(lineParts, lineBytes);
      if (line.at(-1) === 0x0d) line = line.subarray(0, -1);
      try { onEntry?.(JSON.parse(line.toString("utf8"))); } catch {}
    }
    lineParts = [];
    lineBytes = 0;
    lineRelevant = null;
  ...
  for await (const chunk of createReadStream(filePath, { start })) {
    let cursor = 0;
    let lineEnd = chunk.indexOf(0x0a, cursor);
    while (lineEnd >= 0) {
      finishPart(chunk.subarray(cursor, lineEnd), true);
      cursor = lineEnd + 1;
      lineEnd = chunk.indexOf(0x0a, cursor);
    }
    if (cursor < chunk.length) finishPart(chunk.subarray(cursor), false);
  }
  return {
    buffer: lineRelevant === false ? Buffer.alloc(0) : Buffer.concat(lineParts, lineBytes),
    relevant: lineRelevant,
  };
}
...
export class PreviewRepository {
  constructor({
    ...
    maxHistoryEntries = 8,
    maxHistorySourceBytes = 64 * 1024 * 1024,
  } = {}) {
    ...
    this.maxHistorySourceBytes = Math.max(1, maxHistorySourceBytes);
    ...
    this.conversationHistoryCache = new Map();
    this.indexed = false;
    this.efficiencyIndexRefresh = null;
  }
  ...
  async refreshEfficiencyIndex() {
    if (!this.efficiencyIndexRefresh) {
      this.efficiencyIndexRefresh = this.refreshIndex().finally(() => { this.efficiencyIndexRefresh = null; });
    }
    return this.efficiencyIndexRefresh;
  }

  async resolveExactEfficiencyRecord(rawId) {
    const threadId = typeof rawId === "string" ? rawId.match(EXACT_LOCAL_THREAD_ID_PATTERN)?.[1]?.toLowerCase() : null;
    ...
    const current = async () => {
      ...
      try { if (!(await stat(filePath)).isFile()) return null; } catch { return null; }
      return { threadId, metadata, filePath };
    };
    let record = await current();
    if (record) return { ...record, refreshed: false };
    await this.refreshEfficiencyIndex();
    this.sessionWorkspaceById.delete(threadId);
    record = await current();
    return record ? { ...record, refreshed: true } : null;
  }

  /** Exact local identity only: never bind efficiency settings to a same-title thread. */
  async resolveEfficiencyThread(rawId) {
    const record = await this.resolveExactEfficiencyRecord(rawId);
    if (!record) return null;
    let projectPath = await this.sessionWorkspace(record.threadId);
    if (!projectPath && !record.refreshed) {
      await this.refreshEfficiencyIndex();
      this.sessionWorkspaceById.delete(record.threadId);
      if (!this.metadataById.has(record.threadId) || !this.filesById.has(record.threadId)) return null;
      projectPath = await this.sessionWorkspace(record.threadId);
    }
    return { threadId: record.threadId, projectPath: projectPath || null, title: this.metadataById.get(record.threadId)?.title || record.metadata.title };
  }

  async isConversationIdle(rawId, { after = 0 } = {}) {
    const record = await this.resolveExactEfficiencyRecord(rawId);
    if (!record) return false;
    // Unknown/truncated state is not idle. Never rename under an active tool.
    const { lines } = await tailLines(record.filePath, 128 * 1024);
    let idle = false;
    for (const line of lines) {
      let entry; try { entry = JSON.parse(line); } catch { continue; }
      if (entry.type !== "event_msg") continue;
      if (entry.payload?.type === "task_started") idle = false;
      else if (["task_complete", "turn_aborted"].includes(entry.payload?.type)) idle = !after || timestampValue(entry.timestamp) >= after;
    }
    return idle;
  }

  /** Token counts belong to this local session only; account-rate fallbacks are not used. */
  async readTokenUsage(rawId) {
    const record = await this.resolveExactEfficiencyRecord(rawId);
    if (!record) return null;
    try {
      // Empty title deliberately prevents readPreview's legacy title fallback.
      const preview = await this.readPreview(record.threadId);
      if (preview?.threadId !== record.threadId) return null;
      return this.cache.get(record.threadId)?.tokenUsage || null;
    } catch { return null; }
  }

  async readContextSourceRevision(rawId) {
    const record = await this.resolveExactEfficiencyRecord(rawId);
    if (!record) return null;
    const info = await stat(record.filePath);
    return createHash("sha256").update(`${record.threadId}:${info.size}:${info.mtimeMs}`).digest("hex");
  }

  // Draft generation is opt-in/on panel entry, not a full-history scan on each
  // snapshot. Read only a bounded recent tail; the general history cache is
  // size-keyed and cannot prove freshness after an equal-length file rewrite.
  async readContextHistory(rawId) {
    const record = await this.resolveExactEfficiencyRecord(rawId);
    if (!record) return null;
    const sourceRevision = await this.readContextSourceRevision(record.threadId);
    const { lines } = await tailLines(record.filePath, Math.min(this.maxTailBytes, 2 * 1024 * 1024));
    const messages = [];
    for (let index = lines.length - 1; index >= 0 && messages.length < 40; index -= 1) {
      try { const message = parseConversationMessageEntry(JSON.parse(lines[index])); if (message) messages.unshift(message); } catch {}
    }
    return { threadId: record.threadId, title: record.metadata.title, sourceRevision,
      messages: messages.map((entry) => ({ ...entry, text: entry.text.slice(0, 6000),
        contentHash: createHash("sha256").update(entry.text.trim()).digest("hex") })),
      limited: true };
  }
  ...
  async readPreview(rawId, title = "") {
    ...
    this.cache.set(threadId, { cacheKey, preview, usage: parseRateLimitLines(lines), tokenUsage: parseTokenUsageLines(lines) });
    return preview;
  }

  async readConversationHistory(rawId, title = "") {
    await this.ensureIndex();
    let threadId = this.resolveThreadId(rawId, title);
    if (!threadId) {
      await this.refreshIndex();
    ...
    const fileStat = await stat(filePath);
    let cached = this.conversationHistoryCache.get(threadId);
    if (!cached || fileStat.size < cached.size) {
      cached = { size: 0, remainder: null, messages: [], messageIds: new Set() };
    }
    if (fileStat.size > cached.size) {
      const messages = cached.messages.slice();
      const messageIds = new Set(cached.messageIds);
      const remainder = await readConversationAppend(filePath, {
        start: cached.size,
        remainder: cached.remainder,
        onEntry: (entry) => {
          const message = parseConversationMessageEntry(entry);
          if (!message) return;
          const fallbackId = `${message.timestamp || ""}:${message.role}:${messages.length}:${message.text.length}`;
      ...
      cached = { size: fileStat.size, remainder, messages, messageIds };
    }

    // LRU eviction affects only the cache, never stored messages or completeness.
    // Retain the currently requested history even if it alone exceeds the soft
    // source-byte budget, avoiding a repeated full parse of a very long thread.
    this.conversationHistoryCache.delete(threadId);
    this.conversationHistoryCache.set(threadId, cached);
    let cachedSourceBytes = [...this.conversationHistoryCache.values()].reduce((sum, entry) => sum + entry.size, 0);
    while (this.conversationHistoryCache.size > 1 && (this.conversationHistoryCache.size > this.maxHistoryEntries
      || cachedSourceBytes > this.maxHistorySourceBytes)) {
      const oldestId = this.conversationHistoryCache.keys().next().value;
      cachedSourceBytes -= this.conversationHistoryCache.get(oldestId).size;
      this.conversationHistoryCache.delete(oldestId);
    }
    ...
    return {
      threadId,
      title: String(title || metadata?.title || "").trim() || "未命名对话",
      updatedAt: messages.at(-1)?.timestamp || metadata?.updatedAt || new Date(fileStat.mtimeMs).toISOString(),
  ...
  async sessionWorkspace(threadId) {
    ...
    const workspacePath = await readSessionWorkspace(this.filesById.get(threadId), threadId);
