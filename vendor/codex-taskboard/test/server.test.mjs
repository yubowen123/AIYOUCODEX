...
test("accepts private LAN requests and rejects public Host and Origin headers", async () => {
  ...
  assert.equal(codexOriginResult.response.headers.get("access-control-allow-origin"), "app://-");

  const codexBlobOriginResult = await request(baseUrl, "/health", {
    headers: { origin: "null" },
  });
  assert.equal(codexBlobOriginResult.response.status, 200);
  assert.equal(codexBlobOriginResult.response.headers.get("access-control-allow-origin"), "null");
