...
test("embedded blob documents keep taskboard routing in application state", () => {
  assert.equal(canSyncTaskboardHistory("blob:app://-/taskboard-document"), false);
  assert.equal(canSyncTaskboardHistory("http://127.0.0.1:47823/?project=local"), true);
});
...
test("the app restores issue detail from the URL and follows browser history", () => {
  ...
  assert.match(openTaskSource, /canSyncTaskboardHistory\(window\.location\.href\)/);
