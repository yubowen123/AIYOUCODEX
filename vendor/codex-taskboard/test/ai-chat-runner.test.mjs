...
if (args[0] === "app-server") {
  ...
  process.stdin.on("end", () => {
    ...
    if (prompt.includes("ROOT_ERROR_THEN_COMPLETED")) {
      emit({type:"error",message:"Protocol transient warning"});
    }
...
test("root error notices remain non-terminal when Codex later completes", async () => {
  const fixture = await createFixture();
  try {
    const thread = await fixture.service.createThread({ projectId: "project" });
    const run = await fixture.service.startTurn(thread.id, {
      message: "ROOT_ERROR_THEN_COMPLETED",
    });
    await waitFor(() => fixture.service.getRun(run.id).status !== "running");

    assert.equal(fixture.service.getRun(run.id).status, "completed");
    assert.deepEqual(
      fixture.service.getThreadSnapshot(thread.id).events
        .filter((event) => event.content === "Protocol transient warning")
        .map(({ type, role, data }) => ({ type, role, data })),
      [{ type: "runtime_notice", role: "activity", data: { status: "warning" } }],
    );
  } finally {
    await fixture.close();
  }
...
test("item error notices remain non-terminal activity when Codex completes", async () => {
  const fixture = await createFixture();
  try {
    const thread = await fixture.service.createThread({ projectId: "project" });
    const run = await fixture.service.startTurn(thread.id, { message: "ITEM_ERROR" });
    await waitFor(() => fixture.service.getRun(run.id).status !== "running");

    assert.equal(fixture.service.getRun(run.id).status, "completed");
    assert.deepEqual(
      fixture.service.getThreadSnapshot(thread.id).events
        .filter((event) => event.content === "Recoverable item error")
        .map(({ type, role, data }) => ({ type, role, data })),
      [{
        type: "runtime_notice",
        role: "activity",
        data: { status: "completed", itemId: "item-error-1", severity: "warning" },
      }],
    );
  } finally {
    await fixture.close();
