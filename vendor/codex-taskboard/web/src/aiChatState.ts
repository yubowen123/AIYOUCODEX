...
export function activeAiChatRun(
  snapshot: AiChatThreadSnapshot | null,
): AiChatRun | null {
  if (!snapshot) return null;
  const runningRun = snapshot.runs.find((run) => run.status === "running");
  ...
  const recordedRun = snapshot.runs.find((run) => run.id === currentRun.id);
  return recordedRun ? null : currentRun;
}

export function effectiveAiChatThreadStatus(
  snapshot: AiChatThreadSnapshot | null,
): AiChatThreadStatus {
  if (!snapshot) return "idle";
  if (activeAiChatRun(snapshot)) return "running";
  return snapshot.thread.status === "running" ? "idle" : snapshot.thread.status;
}
