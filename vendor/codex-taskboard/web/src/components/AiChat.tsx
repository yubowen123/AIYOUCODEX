...
export function AiChat({ available, projectId, issueId }: AiChatProps) {
  ...
  const currentRun = activeAiChatRun(snapshot);
  const selectedThreadStatus = effectiveAiChatThreadStatus(snapshot);
  ...
  const lastUserEvent = snapshot && selectedThreadStatus === "failed"
