...
interface ProjectActivityState {
  ...
  primaryTaskByProject: Record<string, string>;
  completionByProject: Record<string, ProjectActivityCompletion>;
  seenCompletionByProject: Record<string, string>;
  pinnedProjectIds: string[];
}

interface ProjectActivityCard {
  projectId: string;
  projectName: string;
  taskId: string;
  ...
  phase: "active" | "completed" | "pinned";
  statusLabel: "执行中" | "待查看" | "已钉住";
...
const PROJECT_ACTIVITY_KEY = "taskboard.projectActivity.v1";
...
const PROJECT_ACTIVITY_COMPLETED_STATUSES = new Set<TaskStatus>(["in_review", "done"]);

function emptyProjectActivityState(): ProjectActivityState {
  return {
    version: 1,
    initialized: false,
    lastSyncAt: null,
    activeByProject: {},
    primaryTaskByProject: {},
    completionByProject: {},
    seenCompletionByProject: {},
    pinnedProjectIds: [],
  };
}

function readProjectActivityState(): ProjectActivityState {
  try {
    const value = JSON.parse(window.localStorage.getItem(PROJECT_ACTIVITY_KEY) ?? "null") as Partial<ProjectActivityState> | null;
    if (!value || typeof value !== "object") return emptyProjectActivityState();
    return {
      ...emptyProjectActivityState(),
      ...value,
      version: 1,
      activeByProject: value.activeByProject ?? {},
      primaryTaskByProject: value.primaryTaskByProject ?? {},
      completionByProject: value.completionByProject ?? {},
      seenCompletionByProject: value.seenCompletionByProject ?? {},
      pinnedProjectIds: Array.isArray(value.pinnedProjectIds)
        ? [...new Set(value.pinnedProjectIds.filter((id): id is string => typeof id === "string" && Boolean(id)))].sort()
        : [],
    };
  } catch {
    return emptyProjectActivityState();
  }
}

function projectActivityTaskTime(task: Pick<Task, "updatedAt">): number {
...
function buildProjectActivity(
  projects: Project[],
  tasks: Task[],
  previous: ProjectActivityState,
): { cards: ProjectActivityCard[]; state: ProjectActivityState } {
  const state: ProjectActivityState = structuredClone(previous);
  const firstSync = !previous.initialized;
  const projectById = new Map(projects.map((project) => [project.id, project]));
  const routableTasks = tasks.filter((task) => task.threadId
    && THREAD_ID_PATTERN.test(task.threadId.replace(/^(?:local|cloud):/i, "")));
  const tasksByProject = new Map<string, Task[]>();
  for (const task of routableTasks) {
    tasksByProject.set(task.projectId, [...(tasksByProject.get(task.projectId) ?? []), task]);
  }
  const compareTasks = (left: Task, right: Task) => projectActivityTaskTime(right) - projectActivityTaskTime(left)
  ...
  for (const projectId of projectIds) {
    ...
    const completionTask = projectTasks.find((task) => PROJECT_ACTIVITY_COMPLETED_STATUSES.has(task.status));
    const primaryTask = activeTasks[0] ?? projectTasks[0];
    const wasActive = previous.activeByProject[projectId] === true;
    const isActive = activeTasks.length > 0;
    state.activeByProject[projectId] = isActive;
    if (primaryTask) state.primaryTaskByProject[projectId] = primaryTask.id;
    if (!completionTask) continue;
    const completion = {
      taskId: completionTask.id,
      token: `${completionTask.id}:${completionTask.version}:${completionTask.updatedAt}`,
    };
    const priorCompletion = previous.completionByProject[projectId];
    const completedSinceLastSync = Boolean(previous.lastSyncAt)
      && projectActivityTaskTime(completionTask) > Date.parse(previous.lastSyncAt ?? "");
    const completedWhileAway = !isActive && !wasActive && completedSinceLastSync
      && priorCompletion?.taskId !== completionTask.id;
    if (firstSync) {
      state.completionByProject[projectId] = completion;
      state.seenCompletionByProject[projectId] = completion.token;
    } else if (!isActive && (wasActive || completedWhileAway)) {
      state.completionByProject[projectId] = completion;
  ...
  const cards: ProjectActivityCard[] = [];
  for (const projectId of projectIds) {
    const projectTasks = [...(tasksByProject.get(projectId) ?? [])].sort(compareTasks);
    const activeTasks = projectTasks.filter((task) => task.status === "in_progress");
    const completion = state.completionByProject[projectId];
    const completionUnread = Boolean(completion && state.seenCompletionByProject[projectId] !== completion.token);
    const isPinned = pinned.has(projectId);
    if (activeTasks.length === 0 && !isPinned && !completionUnread) continue;
    const preferredTaskId = activeTasks[0]?.id ?? completion?.taskId ?? state.primaryTaskByProject[projectId];
    const primaryTask = projectTasks.find((task) => task.id === preferredTaskId) ?? activeTasks[0] ?? projectTasks[0];
    if (!primaryTask?.threadId) continue;
    const phase = activeTasks.length > 0 ? "active" : isPinned ? "pinned" : "completed";
    cards.push({
      projectId,
      projectName: projectById.get(projectId)?.name ?? projectId,
      taskId: primaryTask.id,
      taskIdentifier: primaryTask.identifier,
      taskTitle: primaryTask.title,
      threadId: primaryTask.threadId.replace(/^(?:local|cloud):/i, ""),
      activeTaskCount: activeTasks.length,
      phase,
      statusLabel: phase === "active" ? "执行中" : phase === "pinned" ? "已钉住" : "待查看",
      completionToken: completionUnread ? completion?.token ?? null : null,
      pinned: isPinned,
...
interface LocalRealtimeSyncProps {
  ...
  refreshProjectActivity: (signal?: AbortSignal) => Promise<void>;
    ...
    const scheduleRefresh = (options: { projects?: boolean; tasks?: boolean }) => {
      ...
      refreshTimer = window.setTimeout(() => {
        ...
        if (refreshTasksPending) {
          ...
          else void refreshProjectActivity();
        }
...
export function App() {
  ...
  const embedded = query.get("host") === "codex" || window.name === "codex-taskboard-frame";
  ...
  const [projectActivityLoading, setProjectActivityLoading] = useState(true);
  ...
  const [organizingHistory, setOrganizingHistory] = useState(false);
  ...
  const projectActivityRequestRef = useRef(0);
  const projectActivityStateRef = useRef(readProjectActivityState());
  ...
  function openTaskDetail(task: Pick<Task, "identifier" | "projectId">) {
    ...
    const canSyncHistory = canSyncTaskboardHistory(window.location.href);
    if (!currentIssue && canSyncHistory) {
  ...
  const refreshProjectActivity = useCallback(async (signal?: AbortSignal) => {
    const requestId = ++projectActivityRequestRef.current;
    try {
      const nextTasks = await listAllTasks(signal);
      if (requestId === projectActivityRequestRef.current) setProjectActivityTasks(nextTasks);
    } catch (error) {
      if ((error as Error).name !== "AbortError" && requestId === projectActivityRequestRef.current) {
        setLoadError(errorMessage(error));
      }
    } finally {
      if (requestId === projectActivityRequestRef.current) setProjectActivityLoading(false);
    }
  }, []);
  ...
  useEffect(() => {
    ...
    void refreshProjectActivity(controller.signal);
    return () => controller.abort();
  }, [refreshProjectActivity, selectedProjectId]);

  useEffect(() => {
    const result = buildProjectActivity(projects, projectActivityTasks, projectActivityStateRef.current);
    projectActivityStateRef.current = result.state;
    window.localStorage.setItem(PROJECT_ACTIVITY_KEY, JSON.stringify(result.state));
  }, [projectActivityTasks, projects]);
  ...
  function openProjectSwimlaneTask(task: Task) {
    changeProject(task.projectId);
    setDetailTaskIdentifier(task.identifier);
  ...
  async function moveProjectSwimlaneTask(task: Task, status: TaskStatus) {
    if (movingTaskId || task.status === status) return;
    const destination = projectActivityTasks.filter((candidate) => (
      candidate.projectId === task.projectId
      && candidate.status === status
      && candidate.id !== task.id
    ));
    const sortOrder = destination.length > 0
      ? Math.max(...destination.map((candidate) => candidate.sortOrder)) + 1024
      : 1024;
    const previous = task;
    setActionError(null);
    setMovingTaskId(task.id);
    setProjectActivityTasks((current) => current.map((candidate) => (
      candidate.id === task.id ? { ...candidate, status, sortOrder } : candidate
    )));
    try {
      const moved = await moveTaskRequest(task, status, sortOrder);
          ...
          : status === "in_review"
            ...
            : status === "blocked"
              ...
              : status === "done"
                ? "已完成"
                : "已废弃";
      setAnnouncement(`${task.identifier} 已移至${laneLabel}。`);
    } catch (error) {
      setProjectActivityTasks((current) => current.map((candidate) => (
        candidate.id === previous.id ? previous : candidate
      )));
      setActionError(error instanceof ApiError && error.code === "VERSION_CONFLICT"
        ? "该议题已在其他位置更新，总看板正在重新同步。"
        : errorMessage(error));
      void refreshProjectActivity();
    } finally {
      setMovingTaskId(null);
  ...
  async function organizeProjectHistory() {
    if (!selectedProjectId || organizingHistory) return;
    if (!hostContext?.threadId) {
      setActionError("自动化整理需要从当前 Codex 对话中打开项目管理，以便记录操作来源。");
      return;
    }
    setOrganizingHistory(true);
    setActionError(null);
    try {
      const sources = projectChoices.map((choice) => {
        const persisted = projects.find((project) => project.id === choice.id);
        return {
          id: choice.id,
          name: choice.name,
          workspacePath: deviceWorkspacePaths[choice.id] ?? persisted?.workspacePath ?? null,
        };
      });
      const result = await organizeProjectHistoryRequest(
        selectedProjectId,
        hostContext.threadId,
        sources,
      );
      ...
      setAnnouncement(
        result.createdCount > 0
          ? `已自动分类 ${result.sourceCount} 个项目，新增 ${result.createdCount} 个待办。`
          : `已检查 ${result.sourceCount} 个项目，没有新增重复待办。`,
      );
    } catch (error) {
      setActionError(errorMessage(error));
    } finally {
      setOrganizingHistory(false);
    }
  }
        ...
        {selectedProjectId ? (
          ...
          <div className="header-actions">
            {selectedProjectId && boardView === "issues" && (
              <button
                className="history-organize-button"
                type="button"
                disabled={organizingHistory}
                onClick={() => void organizeProjectHistory()}
                title="根据项目类型整理历史项目并加入待办"
              >
                <LinearIcon name="dashboard" />
                <span>{organizingHistory ? "整理中…" : "自动化整理"}</span>
              </button>
            )}
            {selectedProjectId && (
          ...
          <div className="error-banner" role="alert">
            ...
            <button
              ...
              onClick={() => {
                ...
                else void Promise.all([loadProjectList(), refreshProjectActivity()]);
              }}
            >
              Try again
            </button>
          ...
          <section className="project-home">
            ...
            <section className="project-swimlane-section" aria-labelledby="project-swimlane-heading">
              ...
              <ProjectSwimlaneBoard
                ...
                onMoveTask={(task, status) => void moveProjectSwimlaneTask(task, status)}
              />
            </section>
          </section>
