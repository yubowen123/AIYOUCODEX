...
test("efficiency close stays reachable in nested clipped/min-width hosts and after viewport resize", {
  ...
  async function waitForReachable(message) {
    try { await waitForBrowserState(client, reachable, message); } catch (error) {
      const geometry = await client.evaluate(`(()=>{const p=document.getElementById('aiyoucodex-efficiency-panel'),c=p?.querySelector('[data-efficiency-close]'),b=c?.getBoundingClientRect(),hit=b&&document.elementFromPoint(b.x+b.width/2,b.y+b.height/2);return{viewport:{width:innerWidth,height:innerHeight,scale:devicePixelRatio},overlay:p?.dataset.efficiencyViewportOverlay,parent:p?.parentElement?.id,hit:hit&&{tag:hit.tagName,id:hit.id},rects:Object.fromEntries([['panel',p],['close',c],['clipper',document.getElementById('clipper')],['host',document.getElementById('host')],['nativeChat',document.getElementById('native-chat')]].map(([key,node])=>[key,node?.getBoundingClientRect().toJSON()]))}})()`);
      throw new Error(`${error.message}; geometry=${JSON.stringify(geometry)}`, { cause: error });
    }
  }
  await waitForReachable("Initial panel has a reachable close button");
  ...
  await waitForReachable("An unshrinkable native conversation inside a nested transformed clip cannot push close off-screen");
  ...
  await waitForReachable("A narrower viewport recomputes the close hit target and clips width safely");
  await client.evaluate("document.getElementById('clipper').scrollTop=120");
  await waitForReachable("Ancestor scrolling keeps the panel header within the visible clip");
